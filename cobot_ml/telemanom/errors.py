...
class Errors:
    def __init__(self, y_hat, y_test, config):
        """
        ...
        if not len(y_hat) == len(y_test):
            raise ValueError('len(y_hat) != len(y_test): {}, {}'.format(len(y_hat), len(y_test)))
    ...
    def process_batches(self, y_test):
        ...
        for i in range(0, self.n_windows + 1):
            ...
            window = ErrorWindow(y_test, self.config, prior_idx, idx, self, i)
...
class ErrorWindow:
    def __init__(self, y_test, config, start_idx, end_idx, errors, window_num):
        """
        ...
        Args:
            ...
            start_idx (int): Starting index for window within full set of
                test values
            end_idx (int): Ending index for window within full set of
                test values
            errors (arr): Errors class object
            window_num (int): Current window number within test values
    ...
    def prune_anoms(self, inverse=False):
        ...
        i_to_remove = np.array([], dtype=np.int)
